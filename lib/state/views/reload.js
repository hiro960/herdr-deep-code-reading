...
function reloaded(state, message) {
  // Nothing to reload a diff from, and nothing to ask about a merge. The reader's own
  // rows are rebuilt by the caller either way, and the notes are the reason a reload
  // matters most here — an agent's answer lands in a file rather than in a repository.
