...
const { pasteFile, requestDelete, startRename, yankFile } = require("./views/files");
...
function reduceBrowse(state, key, viewport) {
  switch (key) {
    ...
    // And the three it does to a file that already exists, with the other half of the
    // first. `d` is not the delete key: it pages the listing here as it does
    // everywhere, and a reader meaning to scroll must not lose a file for it — see
    // ./views/files, which says what holds all four together.
    case "y":
      return yankFile(state);
    case "p":
      return pasteFile(state);
    case "D":
...
function reduceRead(state, key, viewport) {
  switch (key) {
    ...
    case "*":
      // The same key the diff binds, meaning the same thing: the word under the cursor,
      // looked for from where the cursor is — see dispatch. `n` repeats a search here
      // because nothing else wants the letter, and `*` starts one without typing it.
      return withMatch(state, wordUnderCursor(state), 1, viewport);
    case "n":
...
const REPOSITORY_KEYS = new Set(["e", "tab", "L", "#", "H", "B", "M"]);
const MESSAGE_NO_REPOSITORY = "No repository here — that key reads a history";
// What the diff has instead of a focus. Named here beside the other thing a key says
// when it is bound and has nothing to act on.
const MESSAGE_NO_PANE_FOCUS = "n/p move the file list — j/k always move the lines, so nothing takes focus here";
...
function dispatch(state, key, viewport) {
  ...
  // Except in the browser, where `D` deletes the file under the cursor. `e` opens the
  // diff from there and `D` is waiting in it, so the way home is two keys in the one
  // view that needed the letter for something of its own.
  if (key === WORKING_TREE_KEY && state.view !== VIEW_BROWSE) {
    return state.repository === false
      ? withMessage(state, MESSAGE_NO_REPOSITORY)
      : openWorkingTree(state);
  }
  ...
  // The diff has had no way to look through what is on screen. `/` is the key it is
  // looked through with everywhere else, and it was free here.
  if (key === "/") {
    return { ...clearTransient(state), input: { kind: INPUT_FIND, text: "" } };
  }
  // And `*` is vi's: the word under the cursor, looked for from where the cursor is.
  // It is what stands in for `n` here, which is the file list and never takes a turn
  // at being anything else — and it needs no second key, because the jump lands on
  // another spelling of the same word and the next press carries on from there.
  if (key === "*") {
    return withMatch(state, wordUnderCursor(state), 1, viewport);
  }

  // Nothing below claims Tab, and this is the diff — the one screen with two panes and
  // no focus between them. A reader who has learnt it in the log presses it here and
  // is answered rather than ignored: `j`/`k` always move the lines and `n`/`p` always
  // move the file list, which is what there is instead of a focus to cycle.
  if (key === "tab") {
    return withMessage(state, MESSAGE_NO_PANE_FOCUS);
  }
...
module.exports = { MESSAGE_NO_PANE_FOCUS, MESSAGE_NO_REPOSITORY, reduce };
