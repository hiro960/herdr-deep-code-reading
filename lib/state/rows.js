...
/**
 * Every path the browser may show, asked of whoever can answer.
 *
 * git for a repository — `ls-files` already leaves out `.git` and everything
 * `.gitignore` names. A plain directory is walked instead; see lib/walk, which says
 * what it skips and stops rather than listing for ever.
 *
 * @returns {{paths: Array<string>, truncated: boolean}}
 */
function listPathsOf(state) {
  if (state.repository !== false) {
    return { paths: git.listRepoPaths(state.repoDir), truncated: false };
  }
  return walkFiles(state.repoDir);
}
...
function withRepoPaths(state) {
  ...
  // A walk that had to stop says so: a listing quietly cut reads as the whole of what
  // is there, which is the one thing a listing must not claim falsely.
  const listed = listPathsOf(state);
  const next = { ...state, repoPaths: listed.paths };
  return listed.truncated
    ? { ...next, message: `Listing stopped at ${listed.paths.length} files` }
    : next;
}
