...
function withRepoPaths(state) {
  ...
  // No repository to ask, so the directory is walked instead — see lib/walk, which
  // says what it skips and stops rather than listing for ever. A walk that had to stop
  // says so here: a listing quietly cut reads as the whole of what is there.
  const walked = walkFiles(state.repoDir);
  const listed = { ...state, repoPaths: walked.paths };
  return walked.truncated
    ? { ...listed, message: `Listing stopped at ${walked.paths.length} files` }
    : listed;
}
