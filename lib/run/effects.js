...
const { listPathsOf } = require("../state/rows");
 ...
 */
/**
 * The listing again, with the cursor put back on something.
 *
 * Every key that changes what is in a directory has to answer the same question: the
 * rows in hand are of a tree that has moved. The path it moved to is what the cursor
 * follows where there is one — a copy, a rename — and where there is not, because the
 * file is gone, the cursor keeps the row number it had, clamped to a shorter list.
 */
function relisted(state, focusPath) {
  const listed = listPathsOf(state);
  const dir = state.browse === null || state.browse === undefined ? "" : state.browse.dir;
  const listing = createBrowse(listed.paths, dir);
  const held = state.browse === null || state.browse === undefined ? 0 : state.browse.index;

  const found = listing.entries.findIndex((entry) => entry.path === focusPath);
  const index =
    found !== -1 ? found : Math.max(0, Math.min(held, listing.entries.length - 1));

  return { ...state, effect: null, repoPaths: listed.paths, browse: { ...listing, index } };
}

/** Copy the yanked file to where the browser is standing. */
function performCopy(state, effect) {
  const from = resolveInsideRepo(state.repoDir, effect.from);
  if (!from.ok) {
    return withMessage(state, from.reason);
  }
  const to = resolveNewInsideRepo(state.repoDir, effect.to);
  if (!to.ok) {
    return withMessage(state, to.reason);
  }

  try {
    // COPYFILE_EXCL fails rather than overwrites, which is what closes the moment
    // between asking whether the name is taken and taking it — an agent in another
    // pane writes files too
    fs.copyFileSync(from.path, to.path, fs.constants.COPYFILE_EXCL);
  } catch (error) {
    if (error.code === "EEXIST") {
      return withMessage(state, `${effect.to} already exists`);
    }
    return withMessage(state, `Could not copy to ${effect.to}: ${error.code || error.message}`);
  }

  return withMessage(relisted(state, effect.to), `Copied to ${effect.to}`);
}

/** Delete the file the reader agreed to twice. */
function performDelete(state, effect) {
  const target = resolveInsideRepo(state.repoDir, effect.path);
  if (!target.ok) {
    return withMessage(state, target.reason);
  }

  try {
    fs.unlinkSync(target.path);
  } catch (error) {
    return withMessage(state, `Could not delete ${effect.path}: ${error.code || error.message}`);
  }

  return withMessage(relisted(state, null), `Deleted ${effect.path}`);
}

/** Rename the file under the cursor to what was typed. */
function performRename(state, effect) {
  const from = resolveInsideRepo(state.repoDir, effect.from);
  if (!from.ok) {
    return withMessage(state, from.reason);
  }
  ...
  // rename overwrites where copy can be told not to, so the name is asked about here
  // rather than left to the call. The moment between is the price of a rename.
  if (fs.existsSync(to.path)) {
    return withMessage(state, `${effect.to} already exists`);
  }
  ...
  try {
    ...
    return withMessage(state, `Could not rename to ${effect.to}: ${error.code || error.message}`);
  }

  return withMessage(relisted(state, effect.to), `Renamed to ${effect.to}`);
}
...
const PERFORM = {
  ...
  "copy-file": (state) => performCopy(state, state.effect),
  "delete-file": (state) => performDelete(state, state.effect),
  "rename-file": (state) => performRename(state, state.effect),
