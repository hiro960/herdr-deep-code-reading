...
function createWatcher(session, runtime) {
  ...
  const sync = () => {
    if (timer !== null) {
      seen = watch.fingerprint(session.state.repoDir, session.state.notesFile, session.state.repository);
    }
  };
  ...
  const tick = () => {
    ...
    const now = watch.fingerprint(session.state.repoDir, session.state.notesFile, session.state.repository);
