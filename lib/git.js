...
function resolveRepoRoot(dir) {
  // git failing to start at all — no git on the machine, or a directory that is not
  // there — answers the same question the same way: there is no repository here that
  // this can see. Every caller is asking whether to reach for one, and a throw would
  // be a different answer to the same question.
  let result;
  try {
    result = runGit(dir, ["rev-parse", "--show-toplevel"]);
  } catch {
    return null;
  }
  if (result.status !== 0) {
...
/** Files by the path each is at, so that no change of state reorders the list. */
function inPathOrder(files) {
  return [...files].sort((a, b) => {
    const left = diffPath(a);
    const right = diffPath(b);
...
function loadDiff(repoDir, mode, commit, options) {
  ...
  // In path order, and not in the order the two halves were gathered. `git diff HEAD`
  // reports the tracked changes and knows nothing of the new files, so appending them
  // put every new file at the bottom until it was staged — and staging moved it into
  // the diff, and into path order, which for a name early in the alphabet is the top.
  // The reload follows a file by its path, correctly, so the reader's own keystroke
  // carried their selection away from where they were working: `n`, space, `n`, space
  // walked the list from the beginning again. Nothing a reader stages moves now,
  // because staging does not change a path.
  return { title, files: attachStatus(inPathOrder([...tracked, ...untracked]), entries), branch };
}
